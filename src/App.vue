 ...
 * @LastEditTime: 2021-01-15 10:39:02
 * @Description : 
-->
<template>
  <div id="app">
...
export default {
  ...
  methods: {
    ...
    async getAdministrators() {
      let res = await getAdministratorsApi(
        null,
        'http://106.52.43.100/pd/public/manage.php/Administrator/administrators_message'
      )
      if (res['status'] == 200) {
        localStorage.setItem('administrator_phone', res['message']['phone'])
      } else {
        this.$message.error({
          message: res['message'],
          center: true
