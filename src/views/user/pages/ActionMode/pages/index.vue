 ...
 * @LastEditTime: 2021-01-04 14:33:02
...
export default {
  ...
  methods: {
    ...
    close() {
      ...
      this.$store.dispatch(
        'setActionModeCount',
        this.$store.getters.getActionModeCount - 1
      )
      localStorage.setItem(
        'action_mode_count',
        localStorage.getItem('action_mode_count') - 1
      )
      if (localStorage.getItem('action_mode_count') < 1) {
        this.isOpen1 = true
        this.$message({
          message: '可用次数剩余:' + 0 + '次',
          center: true
        ...
        this.$message.info({
          message:
            '可用次数剩余' + localStorage.getItem('action_mode_count') + '次',
          center: true
        })
      }
    }
