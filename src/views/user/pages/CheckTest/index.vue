 ...
 * @LastEditTime: 2021-01-15 10:27:10
 * @Description : 
-->
<template>
...
export default {
  ...
  methods: {
    ...
    getMessage() {
      ...
      this.axios({
        method: 'post',
        url: 'TrainingMode/trainingMode_record_get_one_api',
        data: this.$store.getters.getTrainId,
        transformRequest: [
          function(data) {
            var oMyForm = new FormData()
            oMyForm.append('train_id', data)
            return oMyForm
          }
        ],
        headers: {
        ...
        .then(response => {
          if (response.data['status'] == 200) {
            ...
            this.checkRound = 1
            this.getData.round = response.data['data']['round']
            this.getData.trainName = response.data['data']['train_name']
            this.getData.testArray = response.data['data']['test_data']
            this.getData.standardArray = response.data['data']['standard_data']
            this.getData.score = response.data['data']['score']
          } else {
            rLoading.close()
            this.$message.error({
        ...
        .catch(error => {
          fsWrite('单个测试数据：（getMessage）' + error)
