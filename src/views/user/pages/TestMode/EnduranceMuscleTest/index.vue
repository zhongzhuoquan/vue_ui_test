 ...
 * @LastEditTime: 2021-01-19 09:13:45
...
export default {
  ...
  methods: {
    ...
    close() {
      ...
      this.axios({
        method: 'post',
        url: 'TestMode/enduranceMuscleTest_record_save_api',
        ...
        transformRequest: [
          function(data) {
            ...
            oMyForm.append('total_time', data.totalTime)
            return oMyForm
          }
        ],
        headers: {
        ...
        .then(response => {
          if (response.data['status'] == 200) {
            ...
            //可训练训练次数减一
            this.$store.dispatch(
              'setEnduranceMuscleCount',
              this.$store.getters.getEnduranceMuscleCount - 1
            )
            localStorage.setItem(
              'endurance_muscle_count',
              localStorage.getItem('endurance_muscle_count') - 1
            )
            if (localStorage.getItem('endurance_muscle_count') < 1) {
              this.isOpen1 = true
              this.$message({
                message: '可用次数剩余:' + 0 + '次',
                center: true
              ...
              this.$message({
                message:
                  ...
                  '可用次数剩余:' +
                  localStorage.getItem('endurance_muscle_count') +
                  '次',
                center: true
              })
            }
