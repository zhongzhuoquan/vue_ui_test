...
export default {
  ...
  methods: {
    ...
    close() {
      ...
      this.$store.dispatch('setDataStatus', 0)
      //可训练训练次数减一
      this.$store.dispatch('setFreeCount', this.$store.getters.getFreeCount - 1)
      localStorage.setItem('free_count', localStorage.getItem('free_count') - 1)
      if (localStorage.getItem('free_count') < 1) {
        this.isOpen1 = true
        this.$message({
          message: '可用次数剩余:' + 0 + '次',
          center: true
        ...
        this.$message.info({
          message: '可用次数剩余' + localStorage.getItem('free_count') + '次',
          center: true
        })
      }
    }
