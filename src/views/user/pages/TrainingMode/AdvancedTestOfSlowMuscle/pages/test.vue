    ...
    open() {
      ...
      this.timeOut = setTimeout(() => {
        ...
        if (this.allRound == 1) {
          ...
          //可训练训练次数减一
          this.$store.dispatch(
            'setAdvancedSlowMuscleCount',
            this.$store.getters.getAdvancedSlowMuscleCount - 1
          )
          localStorage.setItem(
            'advanced_slow_muscle_count',
            localStorage.getItem('advanced_slow_muscle_count') - 1
          )
          if (localStorage.getItem('advanced_slow_muscle_count') < 1) {
            this.isOpen1 = true
            this.$message({
              message: '可用次数剩余:' + 0 + '次',
              center: true
            ...
            this.$message({
              message:
                '可用次数剩余:' +
                localStorage.getItem('advanced_slow_muscle_count') +
                '次',
              center: true
            })
          }
