    ...
    open() {
      ...
      this.timeOut = setTimeout(() => {
        ...
        if (this.allRound == 1) {
          ...
          //可训练训练次数减一
          this.$store.dispatch(
            'setBasicOfSlowMuscleCount',
            this.$store.getters.getBasicOfSlowMuscleCount - 1
          )
          localStorage.setItem(
            'basic_of_slow_muscle_count',
            localStorage.getItem('basic_of_slow_muscle_count') - 1
          )
          if (localStorage.getItem('basic_of_slow_muscle_count') < 1) {
            this.isOpen1 = true
            this.$message({
              message: '可用次数剩余:' + 0 + '次',
              center: true
            ...
            this.$message({
              message:
                '可用次数剩余:' +
                localStorage.getItem('basic_of_slow_muscle_count') +
                '次',
              center: true
            })
          }
