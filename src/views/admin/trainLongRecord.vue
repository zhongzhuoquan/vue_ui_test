 ...
 * @LastEditTime: 2021-01-15 10:23:25
 * @Description : 
-->
<template>
    ...
    <div id="trainingLongRrcord-middio" ref="exportPdf">
      <div id="trainingLongRrcord-middio_block">
        ...
        <el-date-picker
          ...
          type="daterange"
          align="right"
          value-format="yyyy-M-d 00:00:00"
          unlink-panels
          range-separator="至"
  ...
  watch: {
    ...
    sendMessage: {
      handler(newValue, oldValue) {
        ...
        this.axios({
          method: 'post',
          url: 'TrainingMode/basicTestOfSlowMuscle_long_record_get_api',
          ...
          transformRequest: [
            function(data) {
              ...
              oMyForm.append('train_id', newValue.key)
              oMyForm.append('create_time', newValue.value_time)
              return oMyForm
            }
          ],
          headers: {
          ...
          .catch(error => {
            rLoading.close()
            fsWrite('管理员查看长期记录（sendMessage）：' + error)
      ...
      this.axios({
        ...
        transformRequest: [
          function(data) {
            ...
            oMyForm.append('name', data.name)
            oMyForm.append('devices_id', data.devicesId)
            oMyForm.append('train_id', '1')
            oMyForm.append('create_time', data.value_time)
            return oMyForm
          }
        ],
        headers: {
        ...
        .catch(error => {
          rLoading.close()
          fsWrite('管理员查看长期记录（getMessage）：' + error)
          this.$message.error({ message: '网络请求错误', center: true })
        })
    },
