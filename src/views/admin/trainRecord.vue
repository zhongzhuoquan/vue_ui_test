 ...
 * @LastEditTime: 2021-01-15 10:24:28
 * @Description : 
-->
<template>
        ...
        this.axios({
          ...
          transformRequest: [
            function(data) {
              ...
              oMyForm.append('train_id', data.value)
              return oMyForm
            }
          ],
          headers: {
          ...
          .catch(error => {
            fsWrite('管理员查看记录（sendMessage）：' + error)
  ...
  methods: {
    getInformation() {
      ...
      this.axios({
        ...
        .catch(error => {
          rLoading.close()
          fsWrite('管理员查看记录（getInformation）：' + error)
          this.$message.error({ message: '网络请求错误', center: true })
        })
    },
    ...
    handleDelete(index, row) {
      this.$confirm('确定要删除吗？', '提示', {
        ...
        this.axios({
          ...
          .catch(error => {
            rLoading.close()
            fsWrite('管理员查看记录（handleDelete）：' + error)
            this.$message.error({ message: '网络请求错误', center: true })
          })
      })
    },
